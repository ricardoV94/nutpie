...
def _qualifies_for_hyper_param(rv, param) -> bool:
    ...
    axes the param broadcasts along (a hierarchical group) and the param
    carries free-RV randomness for the knob to decouple."""
    return _is_broadcasted(rv, param) and _depends_on_free_rv([param])
...
def _rewrite_model(
    model: Model,
    flow_db_query: RewriteDatabaseQuery,
    db: SequenceDB,
) -> FunctionGraph:
    """Build the model fgraph and run the flow rewrite, leaving the
    ``(Flow|Model)FreeRV`` IR nodes in place so the spec extraction can
    read off each RV's flow, transform, model params, and value var."""
    fgraph, _memo = fgraph_from_model(model)
    db.query(flow_db_query).rewrite(fgraph)
    return fgraph


def _collect_parts(fgraph: FunctionGraph) -> list[_FlowParts]:
    """Per-free-RV :class:`_FlowParts`, in fgraph toposort order."""
    parts = []
    for node in fgraph.toposort():
        p = _flow_node_parts(node)
        if p is not None:
            parts.append(p)
    return parts
...
def reparametrize(
    ...
    """
    fgraph = _rewrite_model(model, flow_db_query, db)
    parts = _collect_parts(fgraph)
